...
# --- ROBUST DATA LOADER (V3.4 DECOUPLED) ---
@st.cache_data
def load_and_process_data():
    ...
    try:
        ...
        # 5. MERGE PIPELINE
        # Merge 1: Market + Specs (High match rate expected)
        df_step1 = pd.merge(df_m, df_s, on=['join_make', 'join_model'], how='inner')
        
        # Merge 2: Result + Prices (Lower match rate expected)
        ...
        # Fix Range (Available in df_step1 AND df_final)
        range_col = next((c for c in df_s.columns if 'range' in c.lower()), None)
        if range_col:
            # Add range to the "Range Only" dataset (df_step1)
            df_step1['Final_Range_Miles'] = df_step1[range_col] * 0.621371
            df_step1 = df_step1[df_step1['Final_Range_Miles'] > 10]
            
            # Add range to the "Full" dataset (df_final)
            df_final['Final_Range_Miles'] = df_final[range_col] * 0.621371
            df_final = df_final[df_final['Final_Range_Miles'] > 10]

        # RETURN: Final(A+B+C), Specs(B), MarketRaw(A), RangeMerge(A+B)
        return df_final, df_s, df_m, df_step1
...
if 'df_main' not in st.session_state:
    with st.spinner("Processing Tri-Dataset Pipeline..."):
        ...
        if df_final is not None:
            st.session_state['df_main'] = df_final        # A+B+C (Strict)
            st.session_state['df_specs'] = df_specs       # Specs Cols
            st.session_state['df_market_raw'] = df_market_raw # A (Raw)
            st.session_state['df_range_merge'] = df_range_merge # A+B (For Range Charts)
            st.rerun()
...
if 'df_main' in st.session_state:
    ...
    c1.metric("Vehicles (Price Verified)", len(df))
